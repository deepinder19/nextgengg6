...
/*import org.opendaylight.groupbasedpolicy.renderer.apic.flow.DestinationMapper;
import org.opendaylight.groupbasedpolicy.renderer.apic.flow.GroupTable;
import org.opendaylight.groupbasedpolicy.renderer.apic.flow.OfTable;
import org.opendaylight.groupbasedpolicy.renderer.apic.flow.OfTable.OfTableCtx;
import org.opendaylight.groupbasedpolicy.renderer.apic.flow.PolicyEnforcer;
import org.opendaylight.groupbasedpolicy.renderer.apic.flow.PortSecurity;
import org.opendaylight.groupbasedpolicy.renderer.apic.flow.SourceMapper;*/
import org.opendaylight.groupbasedpolicy.renderer.apic.sf.SubjectFeatures;
...
public class PolicyManager 
    ...
    private final SingletonTask apicTask;
...
*/
        ...
        apicTask= new SingletonTask(executor, new ApicUpdateTask());
        scheduleUpdate();
        
        LOG.debug("Initialized APic policy manager");
    }
...
//        });
    ...
    public void writeLogs(String str){
    	try{
    	FileWriter fw=new FileWriter(new File(("/root/gbp.log")),true);
    	fw.write(str+"\n");
    	fw.close();
    	}catch(Exception ex){
    ...
    public void policyUpdated(Set<EgKey> updatedConsumers) {
        ...
        scheduleUpdate();
        writeLogs("PolicManager.policyUpdateddddddddddd");
    }
    ...
    private void scheduleUpdate() {
        if (switchManager != null) {
            ...
            apicTask.reschedule(FLOW_UPDATE_DELAY, TimeUnit.MILLISECONDS);
        }
    }
     ...
     */
    private class ApicTask implements Callable<Void> {
        private final Dirty dirty;
        private final NodeId nodeId;

        public ApicTask(Dirty dirty, NodeId nodeId) {
            super();
            this.dirty = dirty;
            this.nodeId = nodeId;
        }
    ...
    private class ApicUpdateTask implements Runnable {
        @Override
        public void run() {
            ...
            for (NodeId node : switchManager.getReadySwitches()) {
                ApicTask swut = new ApicTask(d, node);
                ecs.submit(swut);
                n += 1;
            }
            for (int i = 0; i < n; i++) {
