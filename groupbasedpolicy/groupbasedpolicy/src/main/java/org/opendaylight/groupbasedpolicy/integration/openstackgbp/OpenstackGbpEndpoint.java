...
public class OpenstackGbpEndpoint implements AutoCloseable,
    ...
    private org.opendaylight.yang.gen.v1.urn.opendaylight.groupbasedpolicy.endpoint.rev140421.endpoints.Endpoint translateEndpoint(
        ...
        APicContextBuilder aPicAugmentation = new APicContextBuilder();
        aPicAugmentation.setNodeId(nodeId);
        aPicAugmentation.setNodeConnectorId(nodeConnectorId);
        ...
        newEpBuilder.addAugmentation(APicContext.class,
                aPicAugmentation.build());
    ...
    private void updateAPicEndpoint(final String neutronPortId,
                    ...
                    for (NodeConnector nc : node.getNodeConnector()) {
                        ...
                        if (fcnc.getName().matches(
                            ...
                            LOG.debug("Created Tap:" + fcnc.getName() + ": "
                                    + nc.getId() + " : " + node.getId());
                            updateAPicEndpoint(fcnc.getName(), nc.getId(),
                                    node.getId());
                        }
                    }
                }
