...
public class EndpointManager 
    ...
    protected EndpointBuilder buildEndpoint(RegisterEndpointInput input) {
        APicContextInput ictx = 
                input.getAugmentation(APicContextInput.class);
        return super.buildEndpoint(input)
                .addAugmentation(APicContext.class, 
                                 new APicContextBuilder(ictx).build());
    }
    ...
    private NodeId getLocation(Endpoint endpoint) {
        if (!validEp(endpoint)) 
            return null;
        APicContext context = 
                endpoint.getAugmentation(APicContext.class);
        if (context != null)
